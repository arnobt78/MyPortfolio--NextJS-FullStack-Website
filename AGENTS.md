...
`docs/AGILE_V_PROTOCOL.md` (local, gitignored) or the copy used to start the session.
...
Resume from the latest checkpoint. If a row is `PENDING`, do not synthesize until `APPROVALS.md` has a matching `resume_token`.
...
Next.js 16 loads root `proxy.ts` (`export function proxy`). Do not restore `middleware.ts`.

The proxy sets `x-initial-language` and `x-pathname` for `app/layout.tsx`. Chat `/api/chat` may stay on the Edge runtime; proxy itself is Node-only.
